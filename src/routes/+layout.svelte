<script lang="ts">
	import '../app.css';
	import { session, savedJobsCountStore } from '$lib/session';
	...
	let savedJobsCount: number = $state($session.savedJobsCount);
	...
	const savedJobsCountUnsubscribe = savedJobsCountStore.subscribe((value) => {
		savedJobsCount = value;
	});

	onDestroy(() => {
		sessionUnsubscribe();
		savedJobsCountUnsubscribe();
	});
</script>
	...
	<section class="flex justify-around">
		{#if isLoggedIn}
			<a
				...
				{#if savedJobsCount > 0}
					<span id="savedJobs-counter">
						<span
							class="absolute bottom-14 right-10 z-40 inline-flex content-center items-center justify-center rounded-full bg-blue-600 px-[0.3rem] text-[12px] font-bold text-white"
						>
							{savedJobsCount}
						</span>
					</span>
